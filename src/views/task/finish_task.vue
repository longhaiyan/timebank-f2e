<template>
    <div>
        <div class="my-new-task-list j-my-finish-task-list">
            <el-row v-if="finishTaskInfo&&finishTaskInfo.data" :gutter="10" style="padding: 0 40px;">
                <el-col :sm="12" v-for="item in finishTaskInfo.data" key>
            ...
            <div v-else class="no-task">
                <i class="el-icon-warning"></i>{{finishTaskError}}
...
<script>
  ...
  export default{
    ...
    computed: {
      ...mapState({
        finishTaskInfo: state => state.task.finishTaskInfo,
        finishTaskError: state => state.task.finishTaskError,
      })
    },
    methods: {
      goTaskInfo(taskId) {
    ...
    mounted(){
      this.init()
      console.log("finishTaskInfo", self.finishTaskInfo)
    },
    components: {
